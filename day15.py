...
oxygendroid = intcodecomputer.IntComputer("Oxygen Droid", copy.copy(day15_input),[],False)
...
def changepixel(screen,x,y,newtile):
	screen[y][x] = newtile
	return screen
...
while not Found:
	showscreen(screen)
	movements = ['n','s','w','e']
	print("heading {}, command is {}".format(movements[pointer],movement_commands[movements[pointer]]))

	if (currx,curry) in traveled: print("Repeat!")
	traveled[(currx,curry)] = 1

	#check if there's wall on left hand
	print("Checking {}".format(left_hand[pointer + 1]))
	oxygendroid.inputqueue.append(left_hand[pointer + 1])
	oxygendroid.compute()
	if oxygendroid.lastoutput[0] == 0:
		#put a wall piece on the left
		if pointer == 0: 
			changepixel(screen,currx-1,curry,'#')
		elif pointer == 1: 
			changepixel(screen,currx+1,curry,'#')
		elif pointer == 2: 
			changepixel(screen,currx,curry+1,'#')
		elif pointer == 3: 
			changepixel(screen,currx,curry-1,'#')


		#Walk back and continue forward
		oxygendroid.lastoutput.clear()
		oxygendroid.inputqueue.append(right_hand[pointer + 1])
		oxygendroid.compute()
		oxygendroid.lastoutput.clear()
		oxygendroid.inputqueue.append(movement_commands[movements[pointer]])
		oxygendroid.compute()
		...
		if oxygendroid.lastoutput[0] == 0:
			...
			if pointer == 0: 
				...
				changepixel(screen,currx+1,curry,'#')
				pointer = 1
		#else, no wall, continue same direction
		elif oxygendroid.lastoutput[0] == 1:
			if pointer == 0:
				curry -= 1
			elif pointer == 1:
				curry += 1
		...
		print("Left hand has no wall! Turn!")
		if pointer == 0: #If going north, TURN left
			pointer = left_hand[pointer+1] -1
		elif pointer == 1:
			pointer = left_hand[pointer+1] -1
		elif pointer == 2:
			pointer = left_hand[pointer+1] -1
	...
	oxygendroid.lastoutput.clear()
	update(traveled)
	changepixel(screen,currx,curry,directions(pointer))
	changepixel(screen,21,21,'O')
	os.system('cls')